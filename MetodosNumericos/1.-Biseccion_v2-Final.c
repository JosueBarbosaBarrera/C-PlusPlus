...
void iteracion(){ // Evaluar por iteraciones + error relativo
	...
	if(fx(a) * fx(b) > 0){ // validar si se puede aplicar el metodo de biseccion
		printf("\nNo se puede aplicar el metodo de la biseccion\n"); // en caso de que no aplicara este mensaje
		printf("%f y %f son del mismo signo\n",fx(a) ,fx(b)); // volveremos a introducir datos
	}else{
		printf("\n     a\t\tb       c      f(a)       f(b)       f(c)     Error Relativo\n");
		for(i;i<iter;i++){ // calculo de iteraciones solicitadas por el usuario
			c0 = c;
			c = (a + b) / 2.0; // valor de C
			error=(c-c0)/c;
		if(i==0){
			error=0; // inicio del valor de error 
		}
